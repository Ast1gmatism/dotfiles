...
// Кольцо-индикатор: трек + одноцветная заливка.
// Градиенты — не его забота, для этого есть специализированные обёртки.
Item {
    id: root

    // ── Геометрия ────────────────────────────────────────────────────────
    property real thickness: 8
    property real gapDegrees: 0
    property real radius: width / 2 - thickness / 2
